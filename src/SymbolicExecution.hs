...
execute :: N.Function -> [(SymbolicState, X.Expr)]
...
bindIn getenv envname x e state =
  let env' = getenv state
      tau = typeIn getenv envname x state
   in E.bind x (e, tau) env'
...
bindNew getenv envname x y state =
  let env' = getenv state
   in if env' `E.binds` x
        then throw (AlreadyBound x envname)
        else E.bind x y env'
...
sequence exec (s1, as) =
  let
   ...
   in
    foldl (\branches a -> makeNewStarts branches a >>= exec) start as

function :: SymbolicExecutor N.Function
function (state, N.Function returnty _ params body) =
  let
    state' =
      foldr
        ( \(n, tau) state_i ->
            case tau of
              ...
              T.Integral (T.Ptr _) ->
                let
                  arrTy = T.Complex (T.Arr  (T.base tau) (T.dim tau))
                  arrName = n ++ "_obj"
  ...
  N.DeclareHeapObj t x a ->
    let
     ...
     in
      return (state'', zero)
  N.Declare t x ->
    return (state{rho = bindNewRho x (zero, T.Integral t) state}, zero)
  N.Return e -> exp (state, e)
  _ -> throw (Unsupported "loops")

toOffset :: X.Expr -> X.Expr
toOffset s = case s of
  X.PtrTo _ -> X.BinExpr s AST.Add zero
  X.BinExpr s1 AST.Add s2 -> case (toOffset s1, toOffset s2) of
...
exp (state, e) = case e of
  ...
  N.Assign (N.Var x) e_1 | rho state `E.binds` x -> do
    let tau = typeInRho x state
    (state', s) <- exp (state, e_1)
    return (state'{rho = E.bind x (s, tau) (rho state')}, s)
  N.Assign (N.Index x es) e_v -> do
  ...
  N.Assign{} -> throw (Unsupported "assignment with complex left-hand side")
  N.Deref e_1 -> do
    (state', s) <- exp (state, e_1)
    case toOffset s of
  ...
  N.Index x es ->
    let ptr = valInRho x state
     in case ptr of
          ...
          _ -> throw (NotPointer x)
  N.Int i -> return (state, X.Literal i)
  N.Char c -> return (state, X.Literal (toInteger (fromEnum c)))
  N.FunCall f es -> do
    (state', args) <- many state exp es
    return (state', X.FunCall f args)
  N.Alloc t i a ->
    let
      tau = T.Complex $ T.Arr t i
      state' = state{mu = bindNewMu a (X.NewArr (T.dim tau) (T.base tau), tau) state}
     in
      return (state', X.PtrTo a)
